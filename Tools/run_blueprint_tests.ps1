...
$resultPath = Join-Path $PSScriptRoot "blueprint_test_result.txt"
...
try {
    ...
    & $godotPath --headless --path . --script "res://Tools/test_blueprint.gd" | Out-Null

    if (-not (Test-Path $resultPath)) {
        Write-Host "Blueprint smoke tests did not produce a result file."
        exit 1
    }
