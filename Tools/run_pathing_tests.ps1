...
$resultPath = Join-Path $PSScriptRoot "pathing_test_result.txt"
...
try {
    ...
    & $godotPath --headless --path . --script "res://Tools/test_pathing.gd" | Out-Null

    if (-not (Test-Path $resultPath)) {
        Write-Host "Pathing smoke tests did not produce a result file."
        exit 1
    }
