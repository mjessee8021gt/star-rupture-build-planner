...
$resultPath = Join-Path $PSScriptRoot "save_versioning_test_result.txt"
...
try {
    ...
    & $godotPath --headless --path . --script "res://Tools/test_save_versioning.gd" | Out-Null

    if (-not (Test-Path $resultPath)) {
        Write-Host "Save versioning smoke tests did not produce a result file."
        exit 1
    }
