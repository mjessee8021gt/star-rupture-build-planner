...
$resultPath = Join-Path $PSScriptRoot "debug_layer_test_result.txt"
...
try {
    ...
    & $godotPath --headless --path . --script "res://Tools/test_debug_layers.gd" | Out-Null

    if (-not (Test-Path $resultPath)) {
        Write-Host "Debug layer smoke tests did not produce a result file."
        exit 1
    }
