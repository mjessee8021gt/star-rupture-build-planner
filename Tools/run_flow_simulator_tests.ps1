...
$resultPath = Join-Path $PSScriptRoot "flow_simulator_test_result.txt"
...
try {
    ...
    & $godotPath --headless --path . --script "res://Tools/test_flow_simulator.gd" | Out-Null

    if (-not (Test-Path $resultPath)) {
        Write-Host "FlowSimulator smoke tests did not produce a result file."
        exit 1
    }
