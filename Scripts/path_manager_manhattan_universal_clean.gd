...
func _get_port_center(building: Node, port_path: NodePath) -> Variant:
	...
	# Fallback for custom nodes that expose global_position
...
func _finalize_path(
		from_b: Node2D, from_port: NodePath, from_pos: Vector2,
		to_b: Node2D, to_port: NodePath, to_pos: Vector2
	) -> void:
