...
@onready var input_5_text : Label = $Input5Box/input5Text
...
@onready var input_5_box : ColorRect = $Input5Box
...
@onready var input_5_port := $"Ports/Input 5"
...
var input5_is_pressed := false
var input5_is_connected := false
...
func _ready() -> void:
	...
	$"Ports/Input 5".modulate = Color(0,1,0,0.5)
	...
	input_5_port.pressed.connect(func(): _start_port_drag("Input 5"))
...
func _get_port_global_pos(port_name: String) -> Vector2:
	match port_name:
		...
		"Input 5":
			return input_5_port.global_position + input_5_port.size * 0.5
		_:
...
func _on_recipe_item_selected(index: int) -> void:
	...
	input_5_box.tooltip_text = "unused"
	...
	if recipe:
		...
		if recipe.inputs.size() == 1:
			...
			input_3_text.text = ItemStack.format_qty(recipe.inputs[2].qty)
			input_3_box.tooltip_text = str(recipe.inputs[2].item.display_name)
			input_4_text.text = ItemStack.format_qty(recipe.inputs[3].qty)
...
func _on_input_5_pressed() -> void:
	if not input5_is_pressed:
		if not other_button_pressed:
			$"Ports/Input 5".modulate = Color(0,1,0,1.0)
			input5_is_pressed = true
			other_button_pressed = true


func _on_input_5_mouse_entered() -> void:
	if not input5_is_pressed:
		$"Ports/Input 5".modulate = Color(0, 1, 0, 0.75)


func _on_input_5_mouse_exited() -> void:
	if not input5_is_pressed:
		$"Ports/Input 5".modulate = Color(0, 1, 0, 0.5)
